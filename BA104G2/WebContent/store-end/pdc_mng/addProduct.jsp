...
<div class="col-xs-12 col-sm-5">
	...
	<!-- magic btn -->
	<button id="pdc1" class="btn btn-xs magic">翡翠烏龍</button>
	<button id="pdc2" class="btn btn-xs magic">蜂蜜奶茶</button>
	<button id="pdc3" class="btn btn-xs magic">檸檬汁</button>
	<button id="pdc4" class="btn btn-xs magic">冰淇淋紅茶</button>
	<button id="pdc5" class="btn btn-xs magic">優多綠茶</button>	
...
<!--========================== 功能放這邊 =============================================-->			
	...
	<script>
		...
		$('#pdc1').click(function(){
			...
			$('input[name=discribe]').val("翡翠烏龍");
			$('option[value=PT004]').attr('selected',true);
			$('option[value=已上架]').attr('selected',true);
		});
		$('#pdc2').click(function(){
			$('input[id=com_name]').val("蜂蜜奶茶");
			$('input[name=m_price]').val("45");
			$('input[name=l_price]').val("60");
			$('input[name=discribe]').val("蜂蜜奶茶");
			$('option[value=PT003]').attr('selected',true);
			$('option[value=已上架]').attr('selected',true);
		});
		$('#pdc3').click(function(){
			$('input[id=com_name]').val("檸檬汁");
			$('input[name=m_price]').val("40");
			$('input[name=l_price]').val("55");
			$('input[name=discribe]').val("檸檬汁");
			$('option[value=PT007]').attr('selected',true);
			$('option[value=已上架]').attr('selected',true);
		});
		$('#pdc4').click(function(){
			$('input[id=com_name]').val("冰淇淋紅茶");
			$('input[name=m_price]').val("40");
			$('input[name=l_price]').val("50");
			$('input[name=discribe]').val("冰淇淋紅茶");
			$('option[value=PT010]').attr('selected',true);
			$('option[value=已上架]').attr('selected',true);
		});
		$('#pdc5').click(function(){
			$('input[id=com_name]').val("優多綠茶");
			$('input[name=m_price]').val("35");
			$('input[name=l_price]').val("45");
			$('input[name=discribe]').val("優多綠茶");
			$('option[value=PT009]').attr('selected',true);
